...
public class FileLineDataSourceTest {
	@Test public void test1FileAreas() throws IOException {
		...
		assertEquals("1234567890" + (char) 10, dataSource.get(1));
		assertEquals("" + (char) 10, dataSource.get(2));
		assertEquals("ABCDEFGHIJ" + (char) 10, dataSource.get(3));
		assertEquals("abcde" + (char) 10, dataSource.get(4));
		assertEquals("line with \\r" + (char) 13, dataSource.get(5));
		assertEquals("line with \\n" + (char) 10, dataSource.get(6));
		assertEquals("line with \\r\\n" + (char) 13 + "" + (char) 10, dataSource.get(7));
		assertEquals("ᐊᓕᒍᖅ ᓂᕆᔭᕌᖓᒃᑯ ᓱᕋᙱᑦᑐᓐᓇᖅᑐᖓ", dataSource.get(8));
		assertNull(dataSource.get(9));
	}
}
